...
import icoToSharp from './src/index.ts'
  ...
  describe('image parsing', () => {
    for (const file of files) {
      it(`converts ${file} to sharp-compatible format`, async () => {
        ...
        const ext = file.includes('marvel') ? 'jpg' : 'png'
        writeFileSync(join(outputDir, file.replace('.ico', `.${ext}`)), output)
  ...
  describe('utility functions', () => {
    ...
    it('respects size parameter', async () => {
      const input = new Uint8Array(
        readFileSync(join(fixturesDir, 'github.ico')),
      )
    ...
    it('throws on invalid input', () => {
      expect(() => icoToSharp(new Uint8Array([0, 0, 0]))).toThrow(/Invalid ICO/)
    })
  })
})
